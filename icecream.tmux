...
build_window_icon() {
	local window_status_icon_enable=$(get_tmux_option "@icecream_window_status_icon_enable" "yes")

	local custom_icon_window_last=$(get_tmux_option "@icecream_icon_window_last" "󰖰")
	local custom_icon_window_current=$(get_tmux_option "@icecream_icon_window_current" "󰖯")
	local custom_icon_window_zoom=$(get_tmux_option "@icecream_icon_window_zoom" "󰁌")
	local custom_icon_window_mark=$(get_tmux_option "@icecream_icon_window_mark" "󰃀")
	local custom_icon_window_silent=$(get_tmux_option "@icecream_icon_window_silent" "󰂛")
	local custom_icon_window_activity=$(get_tmux_option "@icecream_icon_window_activity" "󰖲")
	local custom_icon_window_bell=$(get_tmux_option "@icecream_icon_window_bell" "󰂞")
...
load_modules() {
	...
	local modules_custom_path=$PLUGIN_DIR/.
	local modules_status_path=$PLUGIN_DIR/.
	local modules_window_path=$PLUGIN_DIR/.
...
main() {
	local theme
	theme="$(get_tmux_option "@icecream_flavour" "mocha")"
	echo $theme
	...
	# variables.
	# shellcheck source=icecream-frappe.tmuxtheme
	...
	while IFS='=' read -r key val; do
			...
			eval "local $key"="$val"
	done < "${PLUGIN_DIR}/icecream.tmuxtheme"
	...
	local window_left_separator=$(get_tmux_option "@icecream_window_left_separator" "█")
	local window_right_separator=$(get_tmux_option "@icecream_window_right_separator" "█")
	local window_middle_separator=$(get_tmux_option "@icecream_window_middle_separator" "█ ")
	local window_number_position=$(get_tmux_option "@icecream_window_number_position" "left") # right, left
	local window_status_enable=$(get_tmux_option "@icecream_window_status_enable" "no") # right, left
	...
	local status_left_separator=$(get_tmux_option "@icecream_status_left_separator" "█")
	local status_right_separator=$(get_tmux_option "@icecream_status_right_separator" "█")
	local status_right_separator_inverse=$(get_tmux_option "@icecream_status_right_separator_inverse" "no")
	local status_connect_separator=$(get_tmux_option "@icecream_status_connect_separator" "yes")
	local status_fill=$(get_tmux_option "@icecream_status_fill" "icon")

	local status_modules_right=$(get_tmux_option "@icecream_status_modules_right" "application session")
	local loaded_modules_right=$( load_modules "$status_modules_right")

	local status_modules_left=$(get_tmux_option "@icecream_status_modules_left" "")
